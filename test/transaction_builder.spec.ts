...
function constructSign(
  ...
  f.inputs.forEach((input: any, index: number) => {
    ...
    input.signs.forEach((sign: any) => {
      const keyPair = ECPair.fromWIF(sign.keyPair, networkConfig[key]);
      let redeemScript;
      let witnessScript;
      let witnessValue;
...
function construct(
  ...
  const key: (keyof NetworkConfig) = f.network || 'bitcoin';
  const txb = new TransactionBuilder(networkConfig[key]);
  ...
  f.inputs.forEach((input: any) => {
    ...
    if (input.txRaw) {
      ...
      prevTx = Transaction.fromHex(input.txHex, networkConfig[key]);
    } else {
      prevTx = input.txId;
    }
  ...
  describe(`TransactionBuilder: useOldSignArgs === ${useOldSignArgs}`, () => {
    ...
    describe('fromTransaction', () => {
      fixtures.valid.build.forEach(f => {
        it('returns TransactionBuilder, with ' + f.description, () => {
          const key: (keyof NetworkConfig) = f.network || 'bitcoin';
          const tx = Transaction.fromHex(f.txHex, networkConfig[key]);
          ...
          assert.strictEqual(txb.network, networkConfig[key]);
        });
      });
      ...
      it('classifies transaction inputs', () => {
              ...
              const tx = Transaction.fromHex(fixtures.valid.classification.hex, networkConfig[key]);
              const txb = TransactionBuilder.fromTransaction(tx);
    ...
    describe('addInput', () => {
      ...
      it('accepts a prevTx, index [and sequence number]', () => {
        const key: string = 'bitcoin';
        const prevTx = new Transaction(networkConfig[key]);
                 ...
                 () => {
                   ...
                   (f.inputs as any).forEach(
                     (input: any, index: number): void => {
                       input.signs.forEach((sign: any) => {
                         const key: (keyof NetworkConfig) = sign.network || f.network;
                         const keyPair2 = ECPair.fromWIF(sign.keyPair, networkConfig[key]);
    ...
    describe('build', () => {
                       ...
                       assert.throws(() => {
                         ...
                         if (f.txHex) {
                           txb = TransactionBuilder.fromTransaction(
                             Transaction.fromHex(f.txHex, networkConfig[key]),
                           );
                         } else {
                 ...
                 it('for incomplete with 0 signatures', () => {
                   ...
                   const key: string = 'bitcoin';
                   const randomTx = Transaction.fromHex(randomTxData, networkConfig[key]);
                   const txb = new TransactionBuilder();
                 ...
                 it('for incomplete P2SH with 0 signatures', () => {
                   ...
                   const inpTx = Transaction.fromBuffer(inp, networkConfig.testnet);

                   const txb = new TransactionBuilder(networkConfig.testnet);
    ...
    describe('various edge case', () => {
                    ...
                    it('should warn of high fee for segwit transaction based on VSize, not Size', () => {
                      ...
                      const txb = TransactionBuilder.fromTransaction(
                        Transaction.fromHex(rawtx, network),
                      );
                    ...
                    it('should classify witness inputs with witness = true during multisigning', () => {
                      ...
                      TransactionBuilder.fromTransaction(Transaction.fromHex(txHex, network));
                    });
                    ...
                    it('should not classify blank scripts as nonstandard', () => {
                      ...
                      // and, repeat
                      txb = TransactionBuilder.fromTransaction(
                        Transaction.fromHex(incomplete, networkConfig.bitcoin),
                      );
                      txb.addOutput('1Gokm82v6DmtwKEB8AiVhm82hyFSsEvBDK', 15000);
                      txb.sign({
                        prevOutScriptType: 'p2pkh',
