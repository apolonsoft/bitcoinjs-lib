...
const bitcoin_sv_1 = require("./bitcoin-sv");
...
class Psbt {
    ...
    static fromBuffer(buffer, opts = {}) {
        const psbtBase = bip174_2.Psbt.fromBuffer(buffer, transactionFromBufferNetwork);
    ...
    static signToHex(data) {
        const resultInputs = data.inputs.map((input, index) => (Object.assign({}, input, { index, address: bitcore_lib_cash_1.Address.fromString(input.address) })));
        return bitcoin_sv_1.signBSV(Object.assign({}, data, { inputs: resultInputs, keyPairs: data.privateKeys.map(wif => ({
                privateKey: bitcore_lib_cash_1.PrivateKey.fromWIF(wif),
            })) }));
    }
    constructor(opts = {}, data) {
        if (data) {
            ...
            this.data = new bip174_1.Psbt(new PsbtTransaction(Buffer.from([2, 0, 0, 0, 0, 0, 0, 0, 0, 0]), opts.network || DEFAULT_OPTS.network));
        }
        // set defaults
    ...
    addInput(inputData) {
        ...
        if (input.nonWitnessUtxo) {
            addNonWitnessTxCache(this.__CACHE, input, inputIndex, this.__CACHE.__TX.network);
        }
        c.__FEE = undefined;
    ...
    getInputType(inputIndex) {
        const input = utils_1.checkForInput(this.data.inputs, inputIndex);
        const script = getScriptFromUtxo(inputIndex, input, this.__CACHE, this.__CACHE.__TX.network);
    ...
    updateInput(inputIndex, updateData) {
        ...
        if (updateData.nonWitnessUtxo) {
            addNonWitnessTxCache(this.__CACHE, this.data.inputs[inputIndex], inputIndex, this.__CACHE.__TX.network);
        }
        return this;
    }
 ...
 */
const transactionFromBufferNetwork = (buffer, network) => new PsbtTransaction(buffer, network);
/**
...
function checkFees(psbt, cache, opts) {
    ...
    if (opts.network.maximumFeeRate) {
        maxFeeRate = opts.maximumFeeRate > opts.network.maximumFeeRate ? opts.maximumFeeRate : opts.network.maximumFeeRate;
    }
...
function getHashForSig(inputIndex, input, cache, forValidate, sighashTypes) {
    ...
    let prevout;
    if (input.nonWitnessUtxo) {
        const nonWitnessUtxoTx = nonWitnessUtxoTxFromCache(cache, input, inputIndex, unsignedTx.network);
...
function addNonWitnessTxCache(cache, input, inputIndex, network) {
    ...
    cache.__NON_WITNESS_UTXO_TX_CACHE[inputIndex] = transaction_1.Transaction.fromBuffer(input.nonWitnessUtxo, network);
    const self = cache;
...
function getScriptFromUtxo(inputIndex, input, cache, network) {
    if (input.witnessUtxo !== undefined) {
        ...
        const nonWitnessUtxoTx = nonWitnessUtxoTxFromCache(cache, input, inputIndex, network);
...
function pubkeyInInput(pubkey, input, inputIndex, cache) {
    const script = getScriptFromUtxo(inputIndex, input, cache, cache.__TX.network);
